...
To enable search in Mogo, set the option in <tt>environment.rb</tt>. To disable it, simply set the option to false. I've only tested search with MySQL, so set your database connection string to a MySQL database you'll create (default: 'mogo' with user 'root'). Feel free to tune the sample configuration provided in <tt>sphinx.conf</tt> to your taste. You probably won't want to use a passwordless root user in a production environment ;-).

Once that's done, go ahead and run the supplied Rake tasks to get up and running:

  rake sphinx:index        # update the search indexes; you'll probably want to schedule this periodically
