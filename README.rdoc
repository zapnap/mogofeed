...
To enable search in Mogo, set the option in <tt>environment.rb</tt>. I've only tested this with MySQL, so set your database connection string to a MySQL database you'll create (default: 'mogo' with user 'root'). Feel free to tune the sample configuration provided in sphinx.conf to your taste. You probably won't want to use a passwordless root user in a production environment ;-).
